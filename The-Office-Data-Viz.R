...
rating_per_season <- office_df %>% group_by(Season) %>% summarize(ave_per_season = mean(Rating))

# Plot the IMDB rating for every episode of The Office as a line plot
# with date on the X axis and IMDB rating on the Y axis
ggplot(office_df) + geom_point(mapping = aes(x = AirDate, y = Rating))
# Now plot the *average* IMDB rating for *each season* as a scatterplot, a line plot, and both
ggplot(rating_per_season) + geom_point(mapping = aes(x = Season, y = ave_per_season))

ggplot(rating_per_season) + geom_line(mapping = aes(x = Season, y = ave_per_season))

ggplot(rating_per_season) + geom_line(mapping = aes(x = Season, y = ave_per_season)) + geom_point(mapping = aes(x = Season, y = ave_per_season))
