...
total_avg_wealth <- race_wealth %>% filter(type == "Average") %>% group_by(year) %>% summarize(ave_wealth = mean(wealth_family, na.rm = TRUE)) 

# Now let's make a scatter plot of total average family wealth over time

ggplot(data = total_avg_wealth) + geom_point(mapping = aes(x = year, y = ave_wealth))

# Now let's draw a line plot on top of the same scatterplot
ggplot(data = total_avg_wealth) + geom_point(mapping = aes(x = year, y = ave_wealth)) + geom_line(mapping = aes(x = year, y = ave_wealth))
...
ggplot(data = avg_race_wealth) + geom_line(mapping = aes(x = year, y = wealth_family, color = race)) + geom_point(mapping = aes(x = year, y = wealth_family, color = race))
